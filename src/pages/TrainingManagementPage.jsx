...
export default function TrainingManagementPage() {
  ...
  // ⭐ BỔ SUNG: Khai báo Ref cho container cuộn (dù hiện dùng querySelector, vẫn giữ ref không sao)
    ...
    if (scrollContainer) {
      const handleWheel = (e) => {
        ...
        if (e.deltaY !== 0) {
          ...
          // Hệ số 3.5 sẽ làm cuộn nhanh và mượt hơn
    ...
    <Layout>
          ...
          {loading ? (
            ...
            <div className="training-table-container">
              ...
              <div
                ...
                ref={scrollContainerRef} // ⭐ GÁN REF (giữ nguyên)
              >
                <table>
                  <thead>
                    <tr style={{ height: "52px" }}>
                      {headerSubjects.map((s, i) => (
                        // ✅ DÙNG headerSubjects đã sắp xếp
                  ...
                  <tbody>
                    {currentTrainings.map((t, i) => (
                      <tr key={t.internId || i} style={{ height: "52px" }}>
                        {/* ✅ Áp dụng hàm sắp xếp cho scores của mỗi TS */}
                        ...
                        {(t.scores?.length ?? 0) === 0 &&
                          headerSubjects.length === 0 && <td>NA</td>}
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {isEditModalOpen && editingTraining && (
        <EditTrainingModal
          ...
          courseOrder={courseOrder} // ✅ THÊM PROP (để modal hiển thị môn theo thứ tự DB)
        />
      )}
      ...
      <AIAssistantBubble
        ...
        courseOrder={courseOrder} // ✅ THÊM PROP MỚI, KHÔNG ĐỤNG CSS
      />
    </Layout>
  );
}
